...
describe('switchBranchSections — every form the tables hold is one arc', () => {
  it.each(ALL_FORMS.map(f => [f.label, f]))('%s', (_label, form) => {
...
describe('switchStraightLength', () => {
  it('is the symmetric tangent construction of the form’s arc', () => {
    for (const form of ALL_FORMS) {
      const arcLen = switchArcLength(form.R, form.ratio)
      expect(switchStraightLength(form.R, form.ratio))
...
describe('computeSwitchGeometryUtm still builds the same turnout', () => {
  ...
  it('the branch ends a form-arc away from the toe, on either side', () => {
    for (const form of SWITCH_TYPES) {
      for (const side of ['left', 'right']) {
        const g = computeSwitchGeometryUtm(start, 30, form, side, false)
        expect(g.arcLen).toBeCloseTo(switchArcLength(form.R, form.ratio), 12)
        expect(g.branchChain).toHaveLength(1)
